...
{
    //^[0-9]*$  ^(([1-9]\d{0,9})|0)(\.\d{1,2})?$
    NSString *pattern = @"^[0-9]*$";
    NSRegularExpression *regex = [[NSRegularExpression alloc] initWithPattern:pattern options:0 error:nil];
    NSArray *results = [regex matchesInString:allNumberStr options:0 range:NSMakeRange(0, allNumberStr.length)];
    return results.count > 0;
}
#pragma mark -- ***验证输入的都是金额
+(BOOL)ba_isallMoneyCount:(NSString *)moneycount
{
    NSString *pattern = @"^[0-9]+(\\.[0-9]{1,2})?$";
    NSRegularExpression *regex = [[NSRegularExpression alloc] initWithPattern:pattern options:0 error:nil];
    NSArray *results = [regex matchesInString:moneycount options:0 range:NSMakeRange(0, moneycount.length)];
    return results.count > 0;
}
