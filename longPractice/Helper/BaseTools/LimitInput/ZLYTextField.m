...
@interface ZLYTextField ()<UITextFieldDelegate>
        ...
        if([lang isEqualToString:@"zh-Hans"]) { //简体中文输入，包括简体拼音，健体五笔，简体手写
            ...
            if(!position) {
                ...
                if(toBeString.length > self.MaxWordNumber) {
                    ...
                    if (self.textDelegate &&[self.textDelegate respondsToSelector:@selector(textFieldLimitLengh:)]) {
                        [self.textDelegate textFieldLimitLengh:self];
                    }
                }
...
{
    if (self.textDelegate &&[self.textDelegate respondsToSelector:@selector(textFieldReturnKeyPress:)]) {
        [self.textDelegate textFieldReturnKeyPress:self];
    }
    return YES;
}
